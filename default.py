...
line1 = "  ------GRACIAS POR USAR [COLOR cyan]...MAZINGER Z ADDONS...[/COLOR]PROHIBIDA SU VENTA.....[COLOR green][B]CREADOR ING. PALACIOS[/B][/COLOR]'"
...
Base = 'http://k4.usastreams.com:82/storage/499/mazingerz/iniciovip.xml' #    modificar("la ruta de tus listas")

addon = xbmcaddon.Addon('plugin.video.mazinger') #    modificar("el nombre de tu addons")
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.mazingerz.z Lists-%s]: %s" %(addon_version, string)) #    modificar("el nombre de tu addons")
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Mazinger Lists,We failed with error code - "+str(e.code)+",10000,"+icon+")")#    modificar("el nombre de tu addons")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Mazinger list,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")#    modificar("el nombre de tu addons")
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('Mazinger Lists Select A Source', List)   #    modificar("el nombre de tu addons")
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def SearchChannels():
    ...
    List=[]
    List.append(Mazinger)   #    modificar("el nombre de tu addons")
    PassedUrls = 0
    FoundChannel = 1 
    ReadChannel = 0
    FoundMatch = 0
    progress = xbmcgui.DialogProgress()
    progress.create('Mazinger Lists Searching Please wait',' ')  #    modificar("el nombre de tu addons")
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(Mazinger Lists,Please enable Update Commonresolvers to Play in Settings. - ,10000)")  #    modificar("el nombre de tu addons")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Mazinger Lists,Using Urlresolver module.. - ,5000)")  #    modificar("el nombre de tu addons")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Mazinger Lists','Choose a location to save files.',15000,"+icon+")") #    modificar("el nombre de tu addons")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Mazinger Lists', 'Do you want to add this file as a source?') #    modificar("el nombre de tu addons")
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Mazinger Lists Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)' #    modificar("el nombre de tu addons")
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to Mazinger Lists Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)' #    modificar("el nombre de tu addons")
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(Mazinger Lists,No IMDB match found ,7000,"+icon+")") #    modificar("el nombre de tu addons")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Mazinger Lists Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)' #    modificar("el nombre de tu addons")
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Mazinger Lists Favorites','XBMC.RunPlugin(%s)' %fav_params)) #    modificar("el nombre de tu addons")
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Mazinger Lists ,Failed to extract regex. - "+"this"+",4000,"+icon+")") #    modificar("el nombre de tu addons")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Mazinger Lists,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")") #    modificar("el nombre de tu addons")
